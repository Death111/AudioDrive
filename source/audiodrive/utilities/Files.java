...
public class Files {
	...
	public static List<File> list(String directory, boolean recursive) {
		return list(new File(directory), null, recursive);
	}
	...
	public static List<File> list(File directory, boolean recursive) {
		return list(directory, null, recursive);
	}
	...
	public static List<File> list(File directory, String extension, boolean recursive) {
		...
		Arrays.stream(directory.listFiles()).forEach(file -> {
			if (file.isDirectory()) {
				...
				if (extension == null || file.getName().endsWith(extension)) list.add(file);
			}
		});
		return list;
	}
