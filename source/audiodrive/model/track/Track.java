...
public class Track {
	...
	private void calculateSpline() {
		...
		leftBorderVertexBuffer = new VertexBuffer(leftVertexList, true).mode(GL_QUAD_STRIP);
		rightBorderVertexBuffer = new VertexBuffer(rightVertexList, true).mode(GL_QUAD_STRIP);
	...
	private Color getColor(float index) {
		return Color.Lerp(Color.GREEN(), Color.RED(), (float) Arithmetic.linearScale(index, 0, 1, .7f, -.7f));
	}
	...
	private Placement getPlacement(Index index, boolean interpolated, int side) {
		...
		if (side != 0)
			placement.position().add(placement.side().multiplied(Math.signum(side) * width * 2 / 3));
		return placement;
	}
