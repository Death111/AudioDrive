...
public class Track {
	...
	public void update(double time) {
		...
		if (iteration >= audio.getIterationCount())
			iteration = audio.getIterationCount() - 1;
		...
		visibleMusicTowers.forEach(musicTower -> {
			...
			if (musicTower instanceof TubeTower)
	...
	private Placement getPlacement(Index index, boolean interpolated, int rail) {
		...
		if (rail != 0)
			placement.position().add(placement.side().multiplied(Math.signum(rail) * railWidth()));
		return placement;
	}
	...
	public class Index {
		...
		public Index(int integer, double fraction) {
			if (integer > spline.size() - 2)
				integer = spline.size() - 2;
